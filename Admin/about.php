...
if(strlen($_SESSION['alogin']) == 0) {	
    ...
    if ($_POST['submit'] == "Update") {
        ...
        $sql = "UPDATE tblpages SET detail=? WHERE type=?";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param('ss', $pagedetails, $pagetype);
        
        if ($stmt->execute()) {
            $msg = "Page data updated successfully";
        } else {
            $msg = "Error updating page data";
        }
        $stmt->close();
    }
?>
...
<html>
<head>
<title>Crystal Care | About Us Page</title>
...
$pagetype = $_GET['type'];
$sql = "SELECT detail FROM tblpages WHERE type=?";
$stmt = $mysqli->prepare($sql);
